...
def get_degrees(train_dataset_as_list, out_path):
    ...
    if Path(pna_degrees_save_file_path).is_file():
        ...
        for data in tqdm(train_dataset_as_list):
            ei = data.edge_index[[1, 0], :]
            d = degree(ei[1], num_nodes=data.num_nodes, dtype=torch.long)
...
class Estimator(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        if self.edge_dim:
            gnn_args = gnn_args | dict(edge_dim=self.embed_dim)
        ...
        self.output_mlp = nn.Sequential(
            nn.Linear(output_node_dim * 3, 64), nn.BatchNorm1d(64), nn.Mish(), nn.Linear(64, linear_output_size)
        )
    ...
    def forward(self, z, pos, edge_index_radius, batch_mapping):        
        ...
        if self.edge_dim and self.conv_type != "GCN":
            out = self.gnn_model.forward(x=graph_node_feature_batched, edge_index=edge_index_radius, edge_attr=edge_features_source_target)
        else:
            out = self.gnn_model.forward(x=graph_node_feature_batched, edge_index=edge_index_radius)

        # 2. Readout layer (simple global pooling of node features)
        emb_sum_pool = global_add_pool(out, batch_mapping)
        emb_avg_pool = global_mean_pool(out, batch_mapping)
        emb_max_pool = global_max_pool(out, batch_mapping)

        global_emb_pool = torch.cat((emb_sum_pool, emb_avg_pool, emb_max_pool), dim=-1)

        # 3. Apply a final classifier
        predictions = torch.flatten(self.output_mlp(global_emb_pool))

        return out, global_emb_pool, predictions
    ...
    def _batch_loss(self, z, pos, y, edge_index, batch_mapping, step_type):
        # Forward pass (graph_embeddings not used here so far, after forward)
        out, graph_embeddings, predictions = self.forward(z, pos, edge_index, batch_mapping)
        ...
        return task_loss, out, graph_embeddings, predictions


    def _step(self, batch: torch.Tensor, step_type: str):
        assert step_type in ["train", "validation", "test", "validation_test"]

        z, pos, y, edge_index = batch.z, batch.pos, batch.y, batch.edge_index

        total_loss, out, graph_embeddings, predictions = self._batch_loss(
            z, pos, y, edge_index, batch.batch, step_type
        )
