...
def batch_edge_index(edge_index, batch_mapping, num_nodes):
    node_offsets = torch.cat([torch.tensor([0], device=edge_index.device), torch.cumsum(num_nodes, dim=0)[:-1]])

    # Step 2: Expand the offsets for each edge in edge_index
    # You need to apply these offsets based on the graph that each node belongs to
    edge_index_offset = node_offsets[batch_mapping[edge_index]]

    # Step 3: Add the offsets to the concatenated edge_index
    batched_edge_index = edge_index + edge_index_offset
...
class GraphFeatureTokenizer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.proj_node = nn.Linear(self.embed_dim, config.embedding_dim)
        self.proj_edge = nn.Linear(self.embed_dim, config.embedding_dim)
    ...
    def forward(self, batched_data, perturb=None):
        (pos, z, node_num, edge_num, lap_eigvec, edge_index_radius,) = (
            batched_data["pos"],
            batched_data["z"],
        ...
        batch_mapping = torch.repeat_interleave(torch.arange(len(node_num), device=node_num.device), node_num)
        ...
        node_offsets = torch.cat([torch.tensor([0], device=edge_index_radius.device), torch.cumsum(node_num, dim=0)[:-1]])
        repeated_edge_offsets = torch.repeat_interleave(node_offsets, edge_num)
        edge_index_radius_batched = edge_index_radius + repeated_edge_offsets

        graph_node_feature_batched = graph_node_feature[batch_mask]

        edge_batch_mapping = batch_mapping.index_select(0, edge_index_radius_batched[0, :])

        edge_features_source_target = edge_features[edge_batch_mapping, edge_index_radius[0, :], edge_index_radius[1, :], :]

        node_feature = graph_node_feature_batched
        edge_feature = edge_features_source_target

        node_feature = self.proj_node(node_feature)
        edge_feature = self.proj_edge(edge_feature)

        dtype = node_feature.dtype

        padded_index, padded_feature, padding_mask, _, _ = self.get_batch(
            node_feature, edge_index_radius, edge_feature, node_num, edge_num, perturb
        )
