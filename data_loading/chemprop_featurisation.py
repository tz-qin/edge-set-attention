...
def atom_features(atom: Chem.rdchem.Atom, features_constants: Dict[str, Sequence]):
    features = (
...
def atom_features_int(atom: Chem.rdchem.Atom, features_constants: Dict[str, Sequence]):
    features = [
        atom.GetAtomicNum(),
        atom.GetTotalDegree(),
...
def bond_features_int(bond: Chem.rdchem.Bond):
    if bond is None:
        fbond = [1]
    else:
        bt = bond.GetBondType()
