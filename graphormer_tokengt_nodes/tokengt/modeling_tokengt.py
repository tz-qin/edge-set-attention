...
class TokenGTForNodeClassification(TokenGTPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.embedding_dim = config.embedding_dim
        self.tasks_weights = config.tasks_weights
        ...
        self.val_mask = config.val_mask
        self.test_mask = config.test_mask

        self.encoder = NodeTokenGTEncoder(config)
        self.decoder = TokenGTDecoderHead(
            self.embedding_dim,
            config.num_classes,
            not getattr(config, "remove_head", False),
    ...
    def forward(
        ...
        outputs, padded_node_mask = self.encoder(batched_data)
        ...
        loss = None
        loss_fct = CrossEntropyLoss()
        loss = loss_fct(logits_masked.view(-1, self.decoder.num_classes), labels_masked.view(-1))

        # TODO: a specific graph classification output class will have to be created at a later stage
        return SequenceClassifierOutput(loss=loss, logits=logits_masked, hidden_states=None, attentions=None), labels_masked

    def max_nodes(self):
        return self.encoder.max_nodes


class TokenGTNodeEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.dropout_module = torch.nn.Dropout(p=config.dropout)
        self.layerdrop = config.layerdrop
        ...
        self.graph_feature = NodeFeatureTokenizer(config)
        self.performer_finetune = config.performer_finetune
        self.embed_scale = config.embed_scale
    ...
    def forward(
        ...
        if token_embeddings is not None:
            ...
            x, padding_mask, padded_index, padded_node_mask = self.graph_feature(batched_data, perturb)
        ...
        return x, attn_dict, padded_node_mask
    

class NodeTokenGTEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.max_nodes = config.max_nodes
        self.num_embedding_dim = config.num_layers
        ...
        self.node_encoder = TokenGTNodeEncoder(config)
    ...
    def forward(self, batched_data, perturb=None, masked_tokens=None, **unused):
        x, attn_dict, padded_node_mask = self.node_encoder(batched_data, perturb=perturb)
        # x = inner_states[-1].transpose(0, 1)  # batch_size x T x C
        ...
        # project back to size of vocabulary
        if self.share_input_output_embed and hasattr(self.node_encoder.embed_tokens, "weight"):
            x = F.linear(x, self.node_encoder.embed_tokens.weight)
...
class NodeFeatureTokenizer(nn.Module):
    def __init__(self, config):
        super(NodeFeatureTokenizer, self).__init__()
    ...
    def get_batch(node_feature, edge_index, edge_feature, node_num, edge_num, perturb=None):
        ...
        """
        # seq_len = [n + e for n, e in zip(node_num, edge_num)]  # all seq lengths
        seq_len = [n for n in node_num]  # all seq lengths
        batch_size = len(seq_len)
        d = node_feature.size(-1)  # embedding dimension of node features
        max_len = max(seq_len)
        max_n = max(node_num)
    ...
    def forward(self, batched_data, perturb=None):
        ...
        padded_index, padded_feature, padding_mask, padded_node_mask, padded_edge_mask = self.get_batch(
            node_feature, edge_index, edge_feature, node_num, edge_num, perturb
        )
        node_mask = self.get_node_mask(node_num, node_feature.device)  # [batch_size, max(n_node)]
