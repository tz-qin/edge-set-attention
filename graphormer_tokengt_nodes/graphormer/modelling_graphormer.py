...
class GraphormerGraphAttnBias(nn.Module):
    ...
    def forward(
        ...
        # graph_attn_bias = torch.zeros(size=(n_graph, input_nodes.shape[-1], input_nodes.shape[-1]), device=input_nodes.device)
        ...
        if labels is not None:
            ...
            if self.problem_type == "regression":
                if self.num_classes == 1:
                    ...
                    loss = loss_fct(logits.squeeze().view(-1, self.num_classes), labels.squeeze().float().view(-1, self.num_classes))
...
class GraphormerForNodeClassification(GraphormerPreTrainedModel):
    ...
    def __init__(self, config: GraphormerConfig):
        super().__init__(config)
        self.encoder = GraphormerNodeModel(config)
        self.embedding_dim = config.embedding_dim
        self.num_classes = config.num_classes
        self.classifier = GraphormerDecoderHead(self.embedding_dim, self.num_classes)
        self.is_encoder_decoder = True
    ...
    def forward(
        ...
        **unused,
        ...
        # NOTE: this implementation requires a custom trainer.py that uses the "phase" argument to know whether we are
        # in the train, validation, or test loops
        ...
        loss = None
        loss_fct = CrossEntropyLoss()
        loss = loss_fct(logits_masked.view(-1, self.num_classes), labels_masked.view(-1))
        
        return SequenceClassifierOutput(loss=loss, logits=logits_masked, hidden_states=None, attentions=None), labels_masked
    

class GraphormerNodeModel(GraphormerPreTrainedModel):
    def __init__(self, config: GraphormerConfig):
        super().__init__(config)
        self.max_nodes = config.max_nodes

        self.graph_encoder = GraphormerNodeEncoder(config)
    ...
    def forward(
        ...
        **unused,
        ...
        # last inner state, then revert Batch and Graph len
        input_nodes = inner_states.transpose(0, 1)
...
class GraphormerNodeEncoder(nn.Module):
    def __init__(self, config: GraphormerConfig):
        ...
        self.graph_node_feature = GraphormerGraphNodeFeatureForNodeTasks(config)
        self.graph_attn_bias = GraphormerGraphAttnBiasForNodeTask(config)
...
class GraphormerGraphNodeFeatureForNodeTasks(nn.Module):
    """
    Compute node features for each node in the graph.
    """
...
class GraphormerGraphAttnBiasForNodeTask(nn.Module):
    """
    Compute attention bias for each head.
    """
    ...
    def forward(
        ...
        graph_attn_bias[:, :, :, :] = graph_attn_bias[:, :, :, :] + input_edges
        graph_attn_bias = graph_attn_bias + attn_bias.unsqueeze(1)  # reset
