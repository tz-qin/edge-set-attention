...
class CustomLogger(Logger):
    ...
    def classification_binary(self):
        ...
        if true.shape[0] < 1e7:  # AUROC computation for very large datasets is too slow.
            ...
            auroc_score = auroc(
                pred_score.to(torch.device(cfg.device)), true.to(torch.device(cfg.device)).to(torch.long), task="binary"
            )
            if self.test_scores:
                # SK-learn version.
                try:
        ...
        res = {
            ...
            "mcc": reformat(matthews_corrcoef(true, pred_int)),
        }
    ...
    def classification_multi(self):
        ...
        # if true.shape[0] < 1e7:
            ...
            # if self.test_scores:
            #     # SK-learn version.
            #     sk_auc = reformat(roc_auc_score(true, pred_score.exp(), average="macro", multi_class="ovr"))
    ...
    def classification_multilabel(self):
        ...
        acc = torchmetrics.functional.accuracy(
            pred_score, true, task="multilabel", num_classes=cfg.share.dim_out, num_labels=cfg.share.dim_out
        )
        auroc_metric = auroc(
            pred_score.to(torch.device(cfg.device)),
            true.to(torch.device(cfg.device)).to(torch.long),
            task="multilabel",
            num_labels=cfg.share.dim_out,
        )
        ...
        if self.test_scores:
            ...
            ogb = {
                "accuracy": reformat(metrics_ogb.eval_acc(true, (pred_score > 0.0).astype(int))["acc"]),
    ...
    def write_epoch(self, cur_epoch):
        ...
        if self.name == "train":
            stats = {**epoch_stats, **eta_stats, **basic_stats, **task_stats, **custom_stats}
        else:
...
def eval_spearmanr(y_true, y_pred):
    ...
    if y_true.ndim == 1:
        ...
        for i in range(y_true.shape[1]):
            ...
            res_list.append(stats.spearmanr(y_true[is_labeled, i], y_pred[is_labeled, i])[0])
